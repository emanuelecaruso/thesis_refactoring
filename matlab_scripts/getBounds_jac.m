...
DR= [                           cos(db)*cos(dc),                          -cos(db)*sin(dc),          sin(db);
      cos(da)*sin(dc) + cos(dc)*sin(da)*sin(db), cos(da)*cos(dc) - sin(da)*sin(db)*sin(dc), -cos(db)*sin(da);
      sin(da)*sin(dc) - cos(da)*cos(dc)*sin(db), cos(dc)*sin(da) + cos(da)*sin(db)*sin(dc),  cos(da)*cos(db)];
...
mlt=DT*T*b; %T transform cam1 to cam2 (cam1 expressed in cam2)
...
slv_u2_jac = jacobian(slv_u2,[dt0,dt1,dt2,da,db,dc]);
slv_u2_jac = subs(slv_u2_jac,[dt0,dt1,dt2,da,db,dc],[0,0,0,0,0,0]);
slv_u2_jac = simplify(slv_u2_jac,'Steps',1000);
slv_u2_jac = collect(slv_u2_jac,[d1,u1,v1]);
