...
void CandidatesActivator::clearParentIfEmpty( Region* reg){
  ...
  if(reg->subregions_.empty()){
    if( reg->parent_!=nullptr ){
      ...
      std::cout << "porco? " << reg->num_forbid_subregs_  <<  " " << reg->level_ << " " << actptpresencemat_vec_.actptpresencemat_vec_[reg->level_]->mat_[reg->row_][reg->col_] << std::endl;
