...
bool PointsHandler::sampleCandidates(CameraForMapping* cam){


  double t_start=getTime();

  int count = 0;
  int n_pixels_tot = cam->cam_parameters_->resolution_x*cam->cam_parameters_->resolution_y/(pow(4,candidate_level));
  ...
  // int num =(max_num_active_points-cam->points_container_->active_points_projected_.size())+1;
  ...
  // get image
  Image<pixelIntensity>* img_magn_coarse  = new Image<pixelIntensity>(cam->pyramid_->getMagn(candidate_level));
  Image<pixelIntensity>* img_magn2_coarse  = new Image<pixelIntensity>(cam->pyramid_->getMagn2(candidate_level));
  Image<pixelIntensity>* img_magn  = new Image<pixelIntensity>(cam->pyramid_->getMagn(candidate_level));
  ...
  while(true){
    ...
    for (int row=1; row<n_regions_y-1; row++){
      for (int col=1; col<n_regions_x-1; col++){
        ...
        Candidate* cand = new Candidate(cam, pixel, candidate_level);
        ...
        if(adaptive_thresh){
          if(magn_mean){
            ...
            // std::cout << cand->cost_threshold_mapping_ << std::endl;

            cand->cost_threshold_ba_=
            intensity_coeff_ba*((cand->magn_cd_)*mean_magn_coeff_ba + g_th_intensity_ba ) +
            gradient_coeff_ba*((cand->magn_cd2_)*mean_magn_coeff_ba + g_th_gradient_ba );
          }
        }
        else{
        ...
        cam->points_container_->candidates_.push_back(cand);
...
void PointsHandler::collectInvdepths(){
...
int PointsHandler::trackCandidatesReverse(bool groundtruth){
  ...
  for (int i=dso_->cameras_container_->keyframes_active_.size()-2; i>=0; i--){
    ...
    if(groundtruth){
      ...
      sharedCoutDebug("               - not updated: " + std::to_string(n_cands_ep_too_short_));
...
int CandTracker::searchMin( ){
  ...
  for(int i=0; i<ep_segment_.uvs.size(); i++){
    ...
    // float cost=0;
    // if(cand_->magn_cd_ < 0.07){
    //   cost = getCostIntensity(pixel_m);
    // }
    // else{
    //   cost = getCostGradient(pixel_m);
    //   float cost_phase=0;
