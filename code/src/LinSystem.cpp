...
bool Meas::init(ActivePoint* active_point){
  ...
  weight_ = getWeight(active_point);

  if( level_==0 && abs(error) > (active_point->sat_thresh_) ){
...
float Meas::getWeight(ActivePoint* active_point){
  ...
  {
    case HUBER:{
      float huber_thresh = active_point->huber_thresh_;
      if (u<=huber_thresh){
        weight_mest=1./huber_thresh;
      }
      else{
        float rho_der = huberNormDerivative(u,huber_thresh);
  ...
  // float  weight = weight_mest*((1.0/var_)+0.1);
