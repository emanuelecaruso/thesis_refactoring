...
bool Initializer::checkInliersRatio(){
  ...
  int inl = 0;
  for(int i=0; i<inliers_vec.rows; i++){
    if (inliers_vec.at<bool>(i)){
      inl++;
    }
  }

  float inliers_ratio_ = (float)inl/inliers_vec.rows;
  std::cout << "Inliers: " << inl << " out of " << inliers_vec.rows << ", inliers ratio: " << inliers_ratio_ << std::endl;
...
cv::Mat Initializer::findEssentialMatrix(){
  ...
  cv::Mat E = cv::findEssentialMat ( corners_vec_ref_copy, corners_vec_curr,
                                      cv_K, method, prob, threshold, maxIters, inliers_vec );

  // int inl = 0;
  // for(int i=0; i<inliers_vec.rows; i++){
  //   std::cout << inliers_vec.at<bool>(i) << std::endl;
  //   if (inliers_vec.at<bool>(i)){
  //     inl++;
  //   }
  // }
  ...
  // //   std::cout << "ao " << inlier << std::endl;
  // //   if (inlier){
  // //     i++;
  // //   }
  // // }
  // std::cout << "Inliers: " << inl << " out of " << inliers_vec.rows << std::endl;
